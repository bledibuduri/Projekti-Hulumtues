...
model = SARIMAX(train['Temperature'], order=sarima_order, seasonal_order=seasonal_order)
...
r_squared = r2_score(test['Temperature'], predictions)
...
plt.figure(figsize=(14, 7))
plt.plot(test['Temperature'].values, label='Actual')
...
future_df = pd.DataFrame({'Timestamp': future_dates, 'Temperature': future_predictions})

# Print the first few rows of the predictions DataFrame to verify
print(future_df.head())

# Save the predictions to a CSV file
output_path = 'Temperature_predictions_Sarima2024.csv'

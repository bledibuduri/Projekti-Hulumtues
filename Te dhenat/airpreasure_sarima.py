...
plt.plot(df['Air_Pressure'], label='Air Pressure')
...
model = SARIMAX(train['Air_Pressure'], order=sarima_order, seasonal_order=seasonal_order)
...
# Evaluate the model
rmse = np.sqrt(mean_squared_error(test['Air_Pressure'], predictions))
r2 = r2_score(test['Air_Pressure'], predictions)
print(f"RMSE: {rmse}")
print(f"R-squared: {r2}")

# Plot the predictions
plt.figure(figsize=(14, 7))
plt.plot(test['Air_Pressure'].values, label='Actual')
...
future_df = pd.DataFrame({'Timestamp': future_dates, 'Air_Pressure': future_predictions})

# Print the first few rows of the predictions DataFrame to verify
print(future_df.head())

# Save the predictions to a CSV file
output_path = 'AirPressure_SARIMApredictions_2024.csv'

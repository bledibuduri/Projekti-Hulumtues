...
df.rename(columns={'Timestamp': 'ds', 'WindSpeed': 'y'}, inplace=True)

# Plot the time series data
plt.figure(figsize=(14, 7))
plt.plot(df['ds'], df['y'], label='Wind Speed')
...
future_predictions.rename(columns={'ds': 'Timestamp', 'yhat': 'WindSpeed'}, inplace=True)

# Print the first few rows of the predictions DataFrame to verify
print(future_predictions.head())

# Save the predictions to a CSV file
output_path = 'WindSpeed_predictions_prophet2024.csv'

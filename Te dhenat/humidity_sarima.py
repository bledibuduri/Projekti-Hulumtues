...
model = SARIMAX(train['Humidity'], order=sarima_order, seasonal_order=seasonal_order)
...
r_squared = r2_score(test['Humidity'], predictions)
...
plt.figure(figsize=(14, 7))
plt.plot(test['Humidity'].values, label='Actual')
...
future_df = pd.DataFrame({'Timestamp': future_dates, 'Humidity': future_predictions})

# Print the first few rows of the predictions DataFrame to verify
print(future_df.head())

# Save the predictions to a CSV file
output_path = 'Humidity_predictions_sarima_2024.csv'
